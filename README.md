...
两者都**不要求写协议**，内网扫描时直接填 IP 或主机名即可：

| 输入 | 工具的处理 |
|---|---|
| `https://app.corp.local/index.html` | 原样使用 |
| `app.corp.local` | 补成 http 与 https 各一条，一次试全（不用猜目标协议） |
| `192.168.1.10` | 同上 |
| `192.168.1.10:8080` | 同上（端口非 80/443 时两种协议都试） |
| `192.168.1.10:443` | 只按 https 试（端口已经说明了协议） |
| `ftp://host` | 丢弃（只支持 http/https） |

白名单与种子一样宽松：每行一个，也支持逗号 / 中文逗号 / 顿号 / 分号 / 空格分隔。填 IP 或主机名都行，`https://a.com:8443` 这类整条 URL 粘进来也会被自动清洗成 `a.com`。

Web UI 里点「从种子提取」也会按同样规则处理裸 IP 与主机名。

如果种子落在白名单外（会被递归层跳过、表现为任务秒退零节点），界面会直接弹提示告诉你哪些条目越界，不必去日志里翻。
...
| 参数 | 默认 | 说明 |
|------|------|------|
| `-u` / `--urls` | 必填 | URL 清单文件，或者直接给一个 URL / 主机名 / IP（无协议时自动补，见上节） |
...
场景：目标在内网，你所在的跳板机或云桌面**只能进不能出**——既没有外网，也用不了 LLM，结果还带不走。

CLI 加 `--offline`，Web UI 勾上「离线模式」即可。它做三件事：

1. **不调 LLM**。注意这不是能力降级：`core/prefilter.py` 的本地链路（`SECRET_PATTERNS` 硬编码凭证、`LINKFINDER` 接口路径、`VERSION_PATTERNS` 版本指纹、CMS 敏感路径、源码暴露检测、签名机制组合）本来就独立于 LLM，命中结果直接入库，**发现条数一条不少**；少的只是"正则抓不到、需要语义推理"的那部分（越权逻辑、加密误用之类）。
2. **不连代理池**。
3. **渲染阻断白名单外的全部请求**。这是内网场景最容易踩的坑：页面上引用的外网 CDN / 统计 / 字体在无外网时会一直挂起，把 `goto` 拖到超时。实测一个引用了不可达外网资源的页面——不开离线模式时 `Page.goto: Timeout 10000ms exceeded`，**整页渲染失败**、Hook 与交互全部跳过，耗时 15.4 秒；开启离线模式后阻断 4 个外链请求，渲染正常完成，耗时 7.8 秒。

产物全部落在扫描目录，就地可看：

| 文件 | 用途 |
|---|---|
| `report.html` | **自包含单文件，零外链、零依赖，双击用浏览器打开**。每条发现附带命中代码上下文，是云桌面里唯一真正好用的东西 |
| `full.json` | 全量原始数据（机器读） |
| `report.md` | 归档 / 贴进渗透报告 |

部署方式：把整个工具目录拷进云桌面（"只能进"不影响）。运行依赖 Python + httpx/pydantic/PyYAML；Playwright 能装上就用渲染，装不上就在渲染模式选 `off`，本地正则审计照常跑。

```bash
# 内网目标，全程不出网；结果与 report.html 落在 reports/<时间戳>/
python main.py -u urls.txt --domains bank.internal --offline --render hybrid --fresh -o ./dump
```

### 没有 Playwright 时

渲染选 `off` 即可，不影响审计。若选了 hybrid/full 但 Playwright 缺失，工具会检测到并直接跳过（任务开始时会打印状态，不再静默），装了包却没下内核时首次渲染会尝试启动、失败一次后自动降级。
